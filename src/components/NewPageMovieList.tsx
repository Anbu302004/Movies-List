...
import useNewPage from "../hooks/usePageMovies";
...
interface NewPageMovieListProps {
  filterIds?: string[];
}

const NewPageMovieList: React.FC<NewPageMovieListProps> = ({ filterIds }) => {
  const { data: nextweek = [], isLoading: loadingNew, error: errorNew } = useNewPage("Coming Next Week");
  const { data: worthwait = [], isLoading: loadingTrending, error: errorTrending } = useNewPage("Worth the wait");
  const { data: thisweek = [], isLoading: loadingComedy, error: errorComedy } = useNewPage("Coming this week");
  ...
  return (
    <>
      {renderMovies("Coming Next Week", nextweek)}
      {renderMovies("Worth the wait", worthwait)}
      {renderMovies("Coming this week", thisweek)}
    </>
  );
};
      ...
      <div className="popup-content scrollable-popup" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="popup-details">
          ...
          <div className="right-section">
            ...
            <p><strong>Actors:</strong> {movie.actors.map((a) => a.name).join(", ") || "Unknown"}</p>
          </div>
        </div>
      </div>
    </div>
...
export default NewPageMovieList;
