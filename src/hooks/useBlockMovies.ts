...
const fetchBlockMovies = async (blockTitle: string): Promise<Movie[]> => {
    const blocksResponse = await moviesApiClient.get("/blockslist?page_id=1");
    ...
    const block = blocksResponse.data.data.find((block: any) => block.title === blockTitle);
...
const useBlockMovies = (blockTitle: string) => {
    return useQuery<Movie[]>({
        queryKey: ["blockMovies", blockTitle],
        queryFn: () => fetchBlockMovies(blockTitle),
    });
};

export default useBlockMovies;
